...
table = {pattern}.
pattern = wildcard "=" path {";" path} ["+" "RCS"].
wildcard = string.
path = string.
string = nonempty sequence of characters (without " ", "=", ";", "#", "+" or 
         control codes) or
         '"' nonempty sequence of characters (without '"' or control codes) '"'.
whitespace are all characters with an ASCII code below 32/space.
comments are started with '#' and reach until the end of the line.

NOTE: Don't use . or .. as part of paths. ~name/ or ~/ is allowed.
'+RCS' behind a path list will extend the search to RCS subdirectories of the 
given paths.  Whenever a file X isn't found at a particular place xyz/, 
it is checked if a file xyz/RCS/X,v (xyz/X,v) exists.  If so, the search 
is aborted and xyz/RCS/X,v (xyz/X,v) is returned as the file's name.  
This flag has only effect on the operation of FindFile and FindFileExt.
*)
      ...
      BEGIN
        ...
        IF ~r. eof THEN
          IF (ch = '"') THEN             (* parse string enclosed by '"' *)
            ...
            END;
            Files.Read (r, ch)           (* skip terminating '"' *)
...
(* Produces a file name (including the path information) for the file 'fileName'.
   pre: 'redir' is a redirection table initialized with 'Read' (or it is NIL).
     'fileName' (guess what) the name of a file.
   post: 'redir' is searched for the first pattern matching 'fileName'.  The first directory
     associated with that pattern will be used as the file's path. The complete name
     is then stored in 'path'.
     If no pattern is found 'path' will be set to the value in 'fileName'. *)
  VAR
    pattern : Pattern;
  BEGIN
    pattern := redir;
