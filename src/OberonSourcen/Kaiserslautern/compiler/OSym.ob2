...
CONST
  ClusterSize = 256; (* for clustering the pointers *)
  notinlist   = -1;

TYPE
  StructListNode = POINTER TO StructListNodeDesc;
  StructListNodeDesc =
    RECORD
      next : StructListNode;
      items: ARRAY ClusterSize OF OT.Struct;
    END; (* RECORD *)
  StructList =
    RECORD
      lastnum: LONGINT; (* number of the last valid position *)
      cluster: StructListNode;
    END; (* RECORD *)

TYPE
  ModuleListNode = POINTER TO ModuleListNodeDesc;
  ModuleListNodeDesc =
    RECORD
      next : ModuleListNode;
      items: ARRAY ClusterSize OF OT.Object;
    END; (* RECORD *)

  ModuleList =
    RECORD
      lastnum: LONGINT; (* number of the last valid position *)
      cluster: ModuleListNode;
    END; (* RECORD *)
...
PROCEDURE InsertStruct(VAR cluster: StructList; item: OT.Struct);
  VAR
    clpos: LONGINT;
    clnode: StructListNode;
  BEGIN
    INC(cluster.lastnum);
    clpos:=(cluster.lastnum MOD ClusterSize);
    IF (cluster.lastnum = 0)THEN (* first ClusterNode at all *)
      NEW(cluster.cluster); clnode:=cluster.cluster; clnode^.next:=NIL;
    ELSE (* search appropriate ClusterNode *)
      clnode:=cluster.cluster;
      WHILE (clnode^.next # NIL) DO
        clnode:=clnode^.next;
      END; (* WHILE *)
      IF (clpos = 0) THEN (* create new ClusterNode *)
        NEW(clnode^.next); clnode:=clnode^.next; clnode^.next:=NIL;
      END; (* IF *)
    END; (* IF *)
    clnode^.items[clpos]:=item;
  END InsertStruct;
...
PROCEDURE InitStructList(VAR structlist: StructList);
(* Initialize "structlist" by inserting the predeclared 
  ...
  BEGIN
    structlist.cluster:=NIL;
    structlist.lastnum:=-1;
  END InitStructList;

PROCEDURE GetStruct(cluster: StructList; num: LONGINT): OT.Struct;
  VAR
    clnum,clpos: LONGINT;
    clnode: StructListNode;
  BEGIN
    IF (num > cluster.lastnum) THEN
      RETURN(NIL);
    ELSE
      clnum:=num DIV ClusterSize; clpos:=num MOD ClusterSize;
      clnode:=cluster.cluster;
      WHILE (clnum > 0) DO
        DEC(clnum);
        clnode:=clnode^.next;
      END; (* WHILE *)
      RETURN(clnode^.items[clpos]);
    END; (* IF *)
  END GetStruct;

PROCEDURE GetStructNum(cluster: StructList;item: OT.Struct): LONGINT;
  VAR
    found: BOOLEAN;
    clnum,clpos,walk: LONGINT;
    clnode: StructListNode;
  BEGIN
    clnum:=0; clpos:=0; clnode:=cluster.cluster;
    found:=FALSE;
    WHILE ~found & (clnode # NIL) DO
      FOR walk:=0 TO (ClusterSize - 1) DO
        IF (clnode^.items[walk] = item) THEN
          found:=TRUE; clpos:=walk; 
          DEC(clnum); (* will be increased later *)
        END; (* IF *)
      END; (* WHILE *)
      INC(clnum);
      clnode:=clnode^.next;
    END; (* WHILE *)
    IF found THEN
      RETURN((clnum*ClusterSize)+clpos);
    ELSE
      RETURN(notinlist);
    END; (* IF *)
  END GetStructNum;
...
PROCEDURE InitModuleList(VAR cluster: ModuleList);
  BEGIN
    cluster.cluster:=NIL;
    cluster.lastnum:=-1;
  END InitModuleList;

PROCEDURE InsertModule(VAR cluster: ModuleList; item: OT.Object);
  VAR
    clpos: LONGINT;
    clnode: ModuleListNode;
  BEGIN
    INC(cluster.lastnum);
    clpos:=(cluster.lastnum MOD ClusterSize);
    IF (cluster.lastnum = 0) THEN (* first ClusterNode at all *)
...
PROCEDURE GetModule(cluster: ModuleList; num: LONGINT): OT.Object;
  VAR
    clnum,clpos: LONGINT;
    clnode: ModuleListNode;
  BEGIN
    ...
    ELSE
      ...
      clnode:=cluster.cluster;
      WHILE (clnum > 0) DO
        DEC(clnum); clnode:=clnode^.next;
      END; (* WHILE *)
      RETURN(clnode^.items[clpos]);
    END; (* IF *)
  END GetModule;

PROCEDURE GetModuleNum(cluster: ModuleList; item: OT.Object): LONGINT;
  VAR
    found: BOOLEAN;
    clnum,clpos,walk: LONGINT;
    clnode: ModuleListNode;
...
PROCEDURE GetLastModNum(cluster: ModuleList): LONGINT;
  BEGIN
    RETURN(cluster.lastnum);
  END GetLastModNum;
...
(* pre: Symbol file for the module named "module^.name" exists
  ...
  (*        INC(fingerprint.j);  *)
          ...
          fingerprint.i:=0;
        END; (* IF *)
  (*      IF (fingerprint.j = MAX(INTEGER)) THEN
          fingerprint.j:=MIN(INTEGER);
        END; (* IF *) 
  *)
      END NextFingerprint;
