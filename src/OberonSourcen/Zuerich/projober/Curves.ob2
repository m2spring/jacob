MODULE Curves;  (*NW 8.11.90 / 1.2.91*)
   ...
   BEGIN
      IF (f.X <= x) & (x+4 < f.X1) & (f.Y <= y) & (y+4 < f.Y1) THEN Display.ReplConst(col, x, y, 4, 4, 0) END
   END mark;
   ...
   PROCEDURE Read(obj: Graphics.Object; VAR R: Files.Rider; VAR C: Graphics.Context);
      VAR len: INTEGER;
   BEGIN Graphics.ReadInt(R, len); Graphics.ReadInt(R, obj(Curve).kind); Graphics.ReadInt(R, obj(Curve).lw)
   END Read;
   ...
   BEGIN Graphics.WriteObj(W, cno, obj);
      Graphics.WriteInt(W, 4); Graphics.WriteInt(W, obj(Curve).kind); Graphics.WriteInt(W, obj(Curve).lw)
   END Write;
