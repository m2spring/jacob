...
(*<<<<<<<<<<<<<<<
  TYPE LRTRANSLATE = RECORD 
       ...
       RTRANSLATE  = RECORD 
		     ...
		     END;
>>>>>>>>>>>>>>>*)
TYPE CARDINAL = LONGINT; LONGCARD = LONGINT;
     LRTRANSLATEr = RECORD
		     r : LONGREAL;
                    END;
     LRTRANSLATEc = RECORD
  ...
  PROCEDURE Pow10(e:LONGINT): REAL;
  ...
  PROCEDURE LPow10(e: LONGINT): LONGREAL;
  (* Returns 10 to the 'e's.	*)	
    VAR
      expNeg: BOOLEAN;
      b, x: LONGREAL;
  ...
  (* num    = ['+'|'-'] digit {digit}.				*)
    ...
    CONST Integral = 0; Fractional = 1;
    VAR
      i: CARDINAL;
      e, fractionDigits, truncatedDigits: LONGINT;
      phase:LONGINT; 
      
      M: ARRAY HighM+1 OF CARDINAL;
      (* Will hold the DezimalMantissa.	*)
      
      N: ARRAY HighM+1 OF LONGINT;
    ...
    IF (i<LEN(s)) & (s[i]#0X) THEN
      REPEAT
        IF ('0'<=s[i]) & (s[i]<='9') THEN
          ...
          IF (m<HighM) & (M[m]>=MAX(LONGINT) DIV 10) THEN
            (* MUST be MAX(LONGINT) since we're   *)
	    (* calling LFLOAT(x:LONGINT) with it. *)
            INC(m);
          END;
              
          IF M[m]<MAX(LONGINT) DIV 10 THEN
        ...
        END;
        INC(i);
      UNTIL (i>=LEN(s)) OR (s[i]=0X) OR (s[i]='E');
    END;
    
    e:=0; expNeg:=FALSE;
    IF (i<LEN(s)) & (s[i]#0X) & (s[i]='E') THEN
      INC(i);
      IF (i>=LEN(s)) OR (s[i]=0X) THEN RETURN 0.0; END;
      IF s[i]='-' THEN
        INC(i);
        expNeg := TRUE;
      ELSIF s[i]='+' THEN
  ...
  PROCEDURE Real2Str* (x : REAL; n : CARDINAL; k :LONGINT; 
                      VAR S: ARRAY OF CHAR; VAR done: BOOLEAN);
  ...
  (* If k < 0 use scientific notation.			*)
    ...
    VAR
      ...
      c  : CARDINAL;
      neg : BOOLEAN;
      exp, i, p, rexp :LONGINT; 
      blanks, ExpLength,
      n0, i0, k0, exp0 :LONGINT; 

  BEGIN
    done:=FALSE; S[0]:=0X;
    IF LEN(S)<n THEN RETURN END; (* Result won't fit into given String *)
    ...
    IF IsLongRealInfinityOrNaN (x)
	  ...
	  THEN
		...
		S[p  ] := 'N';
	    END;
	    IF p+1 <= SYSTEM.VAL (LONGINT,LEN(S)) THEN S[p+1] := 0X END;
	    done := TRUE;
	  ELSE
	    S[0] := 0X;
	    done := FALSE;
    ...
    IF (blanks<0) & (SYSTEM.VAL(LONGINT,n)-SYSTEM.VAL(LONGINT,LEN(S))>blanks) THEN
    ...
    IF k >= 0 THEN
      ...
      ELSE
        FOR i := 0 TO exp DO
          c := LREAL.LTRUNC (x);
          S[p]:=SYSTEM.VAL (CHAR, ORD ('0') + c); INC(p);
          xl := LREAL.LFLOAT (c);
          x := (x - xl) * 10.0;
        END;
      END;
    ...
    IF k # 0 THEN
      ...
      IF (k > 0) & (exp < 0) THEN
        IF k < ABS (exp) THEN
          FOR i := 1 TO k DO
            S[p]:='0'; INC(p);
          END;
          IF p<=SYSTEM.VAL(LONGINT,LEN(S)) THEN S[p]:=0X; END;
          RETURN
        ELSE
      ...
      IF k < 0 THEN
        ...
        i:=ExpLength-1;
        WHILE i>=0 DO
          S[p+i]:=SYSTEM.VAL(CHAR, ORD('0')+exp MOD 10); exp:=exp DIV 10;
          DEC(i);
        END;
        INC(p,ExpLength);
      END;
