...
TYPE
   ...
   tProcOfFileAddress*	= PROCEDURE (f:IO.tFile; e:tElmt);
...
PROCEDURE Insert*	(VAR List: tList; Elmt: tElmt);
   ...
   BEGIN
      ActElmt := SYSTEM.VAL(tListElmtPtr,Memory.Alloc (SIZE (tListElmt)));
      ActElmt^.Succ := NIL;
      ActElmt^.Elmt := Elmt;
...
PROCEDURE WriteList*	(f: IO.tFile; List: tList; Proc: tProcOfFileAddress);
   BEGIN
