...
PROCEDURE SortFeld(VAR Feld: tFeld);
 ...
 PROCEDURE MergeSort(UnterGrenze, OberGrenze:INTEGER);
 
 VAR MitteIndex:INTEGER; 
 
  PROCEDURE Mischen(UnterGrenze, MitteIndex, OberGrenze:INTEGER);
  VAR i,j,k,l: LONGINT; 
      Temp : tFeld;
  BEGIN (* Mischen *)
   i:=UnterGrenze; 
   j:=MitteIndex+1;
   k:=UnterGrenze; 
   WHILE (i<=MitteIndex) & (j<=OberGrenze) DO
    ...
    INC(k); 
   END; (* WHILE *)
   IF j>OberGrenze THEN 
     j:=OberGrenze;   
     FOR l:=MitteIndex TO i BY -1 DO
      Feld[j]:=Feld[l]; 
      DEC(j); 
     END; (* FOR l *)
   END; (* IF *)
   FOR l:=UnterGrenze TO k-1 DO
    Feld[l]:=Temp[l]; 
   END; (* FOR *)
  END Mischen;
 ...
 BEGIN (* MergeSort *)
  IF UnterGrenze < OberGrenze THEN 
    ...
    MergeSort(UnterGrenze,MitteIndex);
    MergeSort(MitteIndex+1,OberGrenze);
    Mischen(UnterGrenze,MitteIndex,OberGrenze);
  END; (* IF *)
 END MergeSort;
