...
MODULE General;
IMPORT Arguments,System,SYSTEM;

VAR	  MaxAlign*	:LONGINT; 
VAR	  AlignMasks*	: ARRAY 9 OF SET;
...
PROCEDURE Log2*		(x: LONGINT):LONGINT; 
   VAR y:LONGINT; 
   BEGIN
      y := 0;
...
PROCEDURE AntiLog*	(x: LONGINT):LONGINT; 
   VAR y:LONGINT; 
   BEGIN
      y := 0;
...
PROCEDURE Exp10*		(x:LONGINT): REAL;
   VAR
      y		: REAL;
      negative	: BOOLEAN;
   BEGIN
...
BEGIN
   MaxAlign := SYSTEM.ADR (ForAlign.longreal) - SYSTEM.ADR(ForAlign.char);

   AlignMasks [1] := SYSTEM.VAL(SET,0FFFFFFFFH);
   AlignMasks [2] := SYSTEM.VAL(SET,0FFFFFFFEH);
   AlignMasks [3] := SYSTEM.VAL(SET,0FFFFFFFFH);
   AlignMasks [4] := SYSTEM.VAL(SET,0FFFFFFFCH);
   AlignMasks [5] := SYSTEM.VAL(SET,0FFFFFFFFH);
   AlignMasks [6] := SYSTEM.VAL(SET,0FFFFFFFFH);
   AlignMasks [7] := SYSTEM.VAL(SET,0FFFFFFFFH);
   AlignMasks [8] := SYSTEM.VAL(SET,0FFFFFFF8H);

   Arguments.GetArgs (argc, argv);
   System.PutArgs (argc, SYSTEM.ADR(argv^));
END General.
